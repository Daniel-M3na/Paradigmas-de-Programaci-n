...
class Curva:
    ...
    def interpolacion(s,p:int = 0, r:float = 0.0) -> list:
        ...
        #=========================
            ...
            xi.append(zp1*s.x[ip1]+z*s.x[i+j*s.n]+zp2*s.x[ip2+j*s.n])
            for j in range(1,s.dim):
        ...
        #========================
            ...
            am1:np.float64 = a+1.0
            am2:mp.float64 = a+2.0
            ap1:np.float64 = 1.0-a
            ap2:np.float64 = 2.0-a
            ap3:np.float64 = 3.0-a
            z:np.float64 = 1.0*a*a*u12*(-15.0+a*(-35.0+a*(63.0+a*(-25.0))))
            zp1:np.float64 = 1.0*ap1*ap1*u12*(-15.0+ap1*(-35.0+ap1*(63.0+ap1*(-25))))
            ...
            for j in range(1,s.dim):
                ...
                print("La suavidad debe ser 0,1 ó 2")
            return xi

#=================================================================================
# Función zspline crea el Z-spline de un conjunto de conjuntos de dim dimensiones
# n segmentos y continudidad (aproximación) cont
#==================================================================================
